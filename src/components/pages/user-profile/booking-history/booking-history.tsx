...
const BookingHistoryComponent = ({ hallId }: Props) => {
    ...
    const [dataSortCriteria, setDataSortCriteria] = useState<string>("bookingStartDate"); // sort the data based on chosen criteria
    ...
    const handleMoreVertIconClick = (event: React.SyntheticEvent | Event, booking: {_id: string, documentId: string, hallName: string, vendorType: string, customerName: string, customerType: string, eventName: string, bookingStartDateTimestamp: string, bookingDuration: number, bookingStatus: string }) => {
      setAnchorElMap({
        ...anchorElMap,
      ...
      const getUserBookings = async () => {
        ...
        setTimeout(async () => {
          try {
            ...
            const URL =
              userType === "CUSTOMER"
                ? `/api/routes/bookingMaster/getUserBookings/?customerId=${
    ...
    const handleConfirmBooking = async (event: any) => {
      ...
      const bookingMasterRes = await axios.get(
        `/api/routes/bookingMaster/fetchBookingById/${selectedBooking._id}`
      );
      ...
      await axios.patch(`/api/routes/bookingMaster/updateBookingDetails/${selectedBooking._id}`, {
      ...
      <div className={styles.bookingHistory__container}>
        ...
        <div className={styles.wrapper}>
          <div className={styles.secondaryNavbar}>
            <div className={styles['items-list']}>
              <p
                className={`${styles.item} ${currentTab === "ALL" && styles.currentTab}`}
                onClick={() => setCurrentTab("ALL")}
              >
                All Orders
              </p>
              <p
                className={`${styles.item} ${currentTab === "PENDING" && styles.currentTab}`}
                onClick={() => setCurrentTab("PENDING")}
              >
                Pending
              </p>
              <p
                className={`${styles.item} ${currentTab === "UPCOMING" && styles.currentTab}`}
                onClick={() => setCurrentTab("UPCOMING")}
              >
                Upcoming
              </p>
              <p
                className={`${styles.item} ${currentTab === "COMPLETED" && styles.currentTab}`}
            ...
            <div className={styles.calendar}>
              ...
              <button onClick={() => setReloadData(!reloadData)} className={styles.icon} title='reloadBtn'>
                <SearchIcon />
              </button>
            </div>
          </div>
          <div className={styles.tagList}>
            <div className={styles.tag} onClick={() => setDataSortCriteria("bookingId")}>
              <p>Id</p>
            ...
            <div className={styles.tag} onClick={() => setDataSortCriteria("eventType")}>
              <p>Event</p>
              <ArrowDropDownOutlinedIcon className={styles.icon} />
            </div>
            <div
          ...
          <div className={styles['bookings-wrapper']}>
            {isPageLoading ? (
              ...
              userBookings.map((booking: { _id: string, documentId: string, hallName: string, vendorType: string, customerName: string, customerType: string, eventName: string, bookingStartDateTimestamp: string, bookingDuration: number, bookingStatus: string }, index) => (
                <div className={styles.bookingItem} key={index}>
                  <div className={styles['items-list']}>
                    ...
                    {userType === "CUSTOMER" ? (
                      ...
                      <>
                        <div className={styles.item}>{booking.customerName}</div>
                        <div className={styles.item}>{booking.customerType}</div>
                      </>
                    )}
