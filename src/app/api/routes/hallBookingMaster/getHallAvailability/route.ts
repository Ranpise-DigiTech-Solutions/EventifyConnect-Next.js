...
export async function GET(req: NextRequest) {
  ...
  if(!mongoose.Types.ObjectId.isValid(hallObjectId) || !startDateOfWeek || !endDateOfWeek) {
    return new Response(
  ...
  try {
    ...
    const hallBookings = await hallBookingMaster.aggregate([
      ...
      if(!hallBookings || hallBookings.length===0) {
        return new Response(JSON.stringify({ message: "No available slots found for this hall on the specified date range!" }), {
          status: 404,
          headers: { "Content-Type": "application/json" },
        });
      }
